        ...
        if speech_handle.interrupted:
            ...
            if len(message_outputs) > 0:
                ...
                if audio_output is not None:
                    ...
                    # truncate server-side message if the realtime model supports it
                    msg_modalities = await msg_gen.modalities
                    supports_truncation = False
                    try:
                        rm = getattr(self._rt_session, "realtime_model", None)
                        if rm is not None:
                            caps = getattr(rm, "capabilities", None)
                            if caps is not None and getattr(caps, "message_truncation", False):
                    ...
                    if supports_truncation:
                        ...
                        logger.debug(
                            "Realtime model does not support server-side message truncation; skipping truncate()"
                        )
